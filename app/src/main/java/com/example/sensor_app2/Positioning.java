...
public class Positioning {
    ...
    public void start(String measure) {
        ...
        Log.d(TAG, "firstmeasure : " + Arrays.toString(data));

        Matrix results = linearRslt(data);
        ...
        for (int i = 0; i < 5; i++) {
            ...
            for (int j = 0; j < wifiAPManager.apInfoList.size(); j++) {
                if (addr.equals(wifiAPManager.apInfoList.get(j).mac_addr1)) {
                    data[i] = lines[i] + "," + wifiAPManager.apInfoList.get(j).ref_x + "," + wifiAPManager.apInfoList.get(j).ref_y;
                }
